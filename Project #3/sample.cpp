...
const char *WINDOWTITLE = { "OpenGL / GLUT CS_550_Project_3" };
...
float   Time;					// Define Time for Ainmate
int		TextureOn;				// the switch for Texture.
GLuint	texture_earth;			// texture for earth
GLuint	base;
...
MjbSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			{
				p->s = (lng + M_PI) / (2. * M_PI);
				p->t = (lat + sinf(2 * M_PI * (Time + (float)ilng / (float)NumLngs)) + M_PI / 2.) / M_PI;
			}
			else
			{
				p->s = (lng + M_PI) / (2. * M_PI);
...
InitMenus( )
{
	...
	int distortmenu = glutCreateMenu(DoDistortsMenu);
	glutAddMenuEntry("Off", 0);
	glutAddMenuEntry("On", 1);
	...
	glutAddSubMenu(   "Texture On", texturemenu);
...
Textures()
{
	...
	unsigned char* MyTexture = BmpToTexture("test.bmp", &earth_width, &earth_height);
	...
	earth_width = 600;
	earth_height = 600;
	glTexImage2D(GL_TEXTURE_2D, 0, 3, earth_width, earth_height, 0, GL_RGB, GL_UNSIGNED_BYTE, MyTexture);
	//glEnable(GL_TEXTURE_2D);
}
